...
const WishList = () => {
  ...
  let mylist = useSelector((state: any) => state.wishList.List)
    ...
    <>
      ...
      <div className='mt-[200px]'>
        ...
        <div className="w-full flex flex-row items-center justify-center min-h-[50vh] flex-wrap">
          ...
          {mylist.lenght !== 0 ? mylist.map((product: any) => {
            return (
              <div className="flex m-5 w-[240px] h-[450px] flex-row group items-stretch justify-start gap-[16px]" key={`prod-${product.id}`}>
                <div className="relative flex flex-col normal-border w-full leading-[20px] font-semibold">
                  <div className="w-full relative hover: flex flex-col rounded-xl z-0 h-[250px] items-center bg-slate-100 overflow-hidden">
                    <FontAwesomeIcon onClick={(mouse_event, productId = product.productData.id, List = mylist) => {
                      dispatch(addToList({ productId, userId: myuser, List }))
                    }} icon={faHeart} className={`w-[18px] cursor-pointer ${mylist.find((p: any) => p.id === product.id) ? "loved" : "unloved"} h-[18px] absolute right-2 top-2 text-[#cfcfcf] bg-white p-2 rounded-full`} />
                    <Link href={`/productDeta/id?id=${product?.productId}`} className="object-contain w-full"> <Image alt="img" width={240} height={250} src={product.productData.colors[0].images[0]} className="w-full h-[250px]  object-cover rounded-t-lg" />
          ...
          {mylist.length === 0 && myuser ? <div className='p-10 text-primary1 w-full text-[21px] text-center hover:text-[#199aeb]'><Link href="/login">{"you don't have any product here 🤷🏻‍♀️"}</Link> </div> : null}
        </div>
